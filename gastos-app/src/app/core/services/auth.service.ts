      ...
      this.http
        ...
        .subscribe({
          ...
          error: (err) => {
            ...
            const msg =
              err.status === 0
                ? 'API indisponível. Confira se o Docker está rodando (porta 8090).'
      ...
      this.http.post<AuthPayload>(`${this.base}/auth/register`, data).subscribe({
        ...
        error: (err) => {
          const msg =
            err.status === 0
              ? 'API indisponível.'
              : err.error?.error ?? `Erro ${err.status} ao cadastrar.`;
          resolve({ ok: false, message: msg });
        },
      });
    });
    ...
    return new Promise((resolve) => {
      this.http.post<{ message?: string }>(`${this.base}/auth/forgot-password`, { email }).subscribe({
        ...
        error: (err) =>
          resolve({
            ok: false,
            message: err.error?.error ?? 'Não foi possível enviar o e-mail.',
          }),
      });
    });
  }
