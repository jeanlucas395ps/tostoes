...
@Component({
  ...
  host: {
    class: 'year-calendar-host',
    '[class.fill-height]': 'compact()',
  },
})
export class YearCalendarComponent {
  ...
  /** Primeiro mês com dados (created_at do planejamento). */
  planningUsageStart = input<PlanningUsageStart | null>(null);
  /** Layout compacto para uso ao lado do patrimônio na home. */
  compact = input(false);
  ...
  cells = computed((): CalendarMonthCell[] => {
    ...
    return MONTH_LABELS.map((label, index) => {
      ...
      const hasProjected =
        ...
        goals > 0 ||
        leisure > 0;
      const showPreview = hasProjected;
  ...
  selectMonth(index: number): void {
    ...
    if (isBeforePlanningUsageStart(this.year(), index + 1, start)) {
      return;
    }
