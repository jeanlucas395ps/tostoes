...
export class AiReportsComponent implements OnInit, OnDestroy {
  ...
  selectedPreview = computed(() => {
    ...
    return keys.map((key) => {
      ...
      const row =
        y === this.pickerYear()
          ? this.monthsData().find((x) => x.month === m)
          : undefined;
      const preview = this.monthPreviewAmounts(y, m, row);
      return {
        key,
        ...
        beforeStart: !!row?.beforePlanningStart,
      };
    });
  });
  ...
  remove(item: AiReportListItem, event?: Event): void {
    event?.stopPropagation();
    if (!confirm(`Remover o relatório "${item.title}"?`)) return;
  ...
  monthPreview(month: number): {
    ...
    return this.monthPreviewAmounts(this.pickerYear(), month, this.monthCell(month));
  }

  private monthPreviewAmounts(
    year: number,
    month: number,
    row: MonthSummary | undefined
  ): { income: number; expense: number; balance: number; forecast: boolean } {
    const now = new Date();
    const isFuture =
    ...
    const hasReal =
      ...
      Math.abs(row?.balanceReal ?? 0) > 0.001;
    if (!hasReal && (row?.projected.income || row?.projected.expense)) {
      return {
        income: row?.projected.income ?? 0,
        expense: row?.projected.expense ?? 0,
        balance: row?.balanceProjected ?? 0,
