...
export class PlanningService {
  ...
  setActive(id: number): void {
    this.storage.set(STORAGE_KEYS.planningId, String(id));
    this.activeId.set(id);
  }

  clear(): void {
    this.storage.remove(STORAGE_KEYS.planningId);
  ...
  update(id: number, name: string): Observable<Planning> {
    return this.http.put<{ item: Planning }>(`${this.base}/plannings/${id}`, { name }).pipe(
      tap((res) => {
        this.items.update((list) => list.map((p) => (p.id === id ? res.item : p)));
      }),
      map((res) => res.item)
    );
  }
  ...
  sendInvite(
    ...
    return this.http.post<{
      ...
      planningName?: string;
      inviteUrl?: string;
    }>(`${this.base}/plannings/${planningId}/invites`, { email, planningId });
  }
  ...
  private readStoredId(): number | null {
    const raw = this.storage.get(STORAGE_KEYS.planningId);
    if (!raw) return null;
